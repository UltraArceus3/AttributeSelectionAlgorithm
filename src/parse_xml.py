...
"""
    FUNCTION parse_xml
    OUTPUT: DOES NOT RETURN but generats xml file (config_text.xml) which acts as configuration file for RLA code.
    The file is generated inside data folder. This needs to be done before processed data generation.
    function called by call_RLA() function in main.py file.
...
def parse_xml():
    ...
    """
        Iterate through sample_output attribute from config.yaml (which contains path for input files after sampling) and write to xml
    """
    ...
    version_conf = root.new_tag(
        "version-config-param", attrs={"id": "ComparisonGroup"})
    ...
    for i in range(len(comp)):
        ...
        attribute_indices = root.new_tag(
            "comparing_attribute_indices", attrs={"id": "1"})
